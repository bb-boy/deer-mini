// Adapted from DeerFlow frontend/src/components/ai-elements/reasoning.tsx; license: src/markdown/DEERFLOW-LICENSE.
"use client";
...
import { reasoningPlugins } from "../../markdown/plugins";
...
export const Reasoning = memo(
  ({
    ...
    // 用户手动选择后不再自动收起；重新分组时也保留父组件记录的选择。
    ...
    useEffect(() => {
      if (autoClose && defaultOpen && !isStreaming && isOpen && !hasAutoClosed && !userToggled.current) {
...
export const ReasoningContent = memo(
  ({ className, children, ...props }: ReasoningContentProps) => {
    ...
    <CollapsibleContent
      ...
      {...props}
    >
      <ClipboardSafeStreamdown {...reasoningPlugins} mode={isStreaming ? "streaming" : "static"} isAnimating={isStreaming}>
        {children}
